...
namespace cs296
{
	...
	b2Body* ball;
	b2Body* rod2[4], * rod1[4];///< Bodies of rods
	...
	/**Handle keyboard inputs*/
		void dominos_t::keyboard(unsigned char key){
			std::cout<<key<<endl;
			int mul=6;/// Variable mul: controls force of swing
			switch(key){
			///Team 2
			case'e': 
			...
			case'1':///Keypress 1:Stops the rod 
				...
				break;
			case 'a':///Keypress a: Rotates the player towards left,if the player is already to the left and a is pressed then the player get lifted
				...
				break;
			case 'd':///Keypress d: Rotates the player towards right,if the player is already to the right and d is pressed then the player get lifted
				for(int i=range[rod_2_index][0];i<range[rod_2_index][1];i++){
					b2Body *bod= m_joint_2[i]->GetBodyA();
					b2Vec2 v1=m_joint_2[i]->GetAnchorB();
					b2Vec2 v2=m_joint_2[i]->GetAnchorA();
			...
			case 'j':
				...
				break;
			case 'h':///Keypress h: Rotates the player towards left,if the player is already to the left and h is pressed then the player get lifted
				...
				break;
			case 'k':///Keypress k: Rotates the player towards right,if the player is already to the right and k is pressed then the player get lifted
				for(int i=range[rod_1_index][0];i<range[rod_1_index][1];i++){
					b2Body *bod= m_joint_1[i]->GetBodyA();
					b2Vec2 v1=m_joint_1[i]->GetAnchorB();
					b2Vec2 v2=m_joint_1[i]->GetAnchorA();
...
/////////////////////////////////////////////////////////////////////////////
	...
	///a. Team 1
	{
		...
		///Rod 1		
		bd_player1[0].position.Set(-28.0f,20.0f);
...
/////////////////////////////////////////////////////////////////////////////
		...
		///Rod 1		
		bd_player2[0].position.Set(28.0f,20.0f);
